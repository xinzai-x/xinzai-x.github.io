  ...
  if (giscusBtn) {
    giscusBtn.addEventListener('click', function () {
      ...
      if (container) {
        ...
        <% if (theme.comments && theme.comments.giscus) { %>
        var giscusScript = document.createElement('script');
        giscusScript.src = 'https://giscus.app/client.js';
        giscusScript.setAttribute('data-repo', '<%= theme.comments.giscus.repo %>');
        giscusScript.setAttribute('data-repo-id', '<%= theme.comments.giscus.repo_id %>');
        giscusScript.setAttribute('data-category', '<%= theme.comments.giscus.category %>');
        giscusScript.setAttribute('data-category-id', '<%= theme.comments.giscus.category_id %>');
        giscusScript.setAttribute('data-mapping', '<%= theme.comments.giscus.mapping || "pathname" %>');
        giscusScript.setAttribute('data-strict', '<%= theme.comments.giscus.strict || "0" %>');
        giscusScript.setAttribute('data-reactions-enabled', '<%= theme.comments.giscus.reactions_enabled || "1" %>');
        giscusScript.setAttribute('data-emit-metadata', '<%= theme.comments.giscus.emit_metadata || "0" %>');
        giscusScript.setAttribute('data-input-position', '<%= theme.comments.giscus.input_position || "bottom" %>');
        giscusScript.setAttribute('data-lang', '<%= theme.comments.giscus.lang || "zh-CN" %>');
        giscusScript.setAttribute('data-loading', '<%= theme.comments.giscus.loading || "lazy" %>');
